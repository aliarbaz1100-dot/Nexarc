(function(){
  ...
  function cleanCatalog(data){var pool=Array.isArray(data)?data:((data&&data.games)||[]),seenUrl={},seenId={},clean=[],dropped=[];pool.forEach(function(g){if(clean.length>=120)return;var ok=validGame(g)&&!seenUrl[g.embedUrl]&&!seenId[g.id];if(!ok){dropped.push((g&&g.id)||'unknown');return}seenUrl[g.embedUrl]=1;seenId[g.id]=1;clean.push(g)});window.NEXARC_CATALOG_STATS={loaded:clean.length,dropped:dropped,target:120,fullyTested:Number((data&&data.fullyTested)||0),source:'single-master-games-json'};return clean}
  ...
  function gamesByIds(ids){return ids.map(function(id){for(var i=0;i<allGames.length;i++){if(allGames[i].id===id)return allGames[i]}return null}).filter(Boolean)}
  ...
  function loadCatalog(){
    ...
    fetch('/games.json?v=20260912-20664stable2&t='+Date.now(),{cache:'no-store'}).then(function(res){if(!res.ok)throw new Error('games.json '+res.status);return res.json()}).then(function(data){
      allGames=cleanCatalog(data);
      if(!allGames.length)throw new Error('empty catalog');
      applyInitialState();startHero();hideIntro();
      if(window.nexarcStatus)window.nexarcStatus('NEXARC ready','Game library loaded successfully.');
      console.info('[NEXARC] '+allGames.length+' source-verified games loaded.',window.NEXARC_CATALOG_STATS);
    }).catch(function(e){
      console.error('[NEXARC] catalog failed',e);
      if(count)count.textContent='Tap retry to load games';
      if(rows)rows.innerHTML='<div class="emptyState">Games could not load. <button id="catalogRetry" class="secondary">Retry</button></div>';
      var r=byId('catalogRetry');if(r)r.onclick=loadCatalog;
      hideIntro();
    })
  }
  ...
  function init(){
    ...
    window.nexarcPlayRandom=function(){if(allGames.length)openGame(allGames[Math.floor(Math.random()*allGames.length)])};
